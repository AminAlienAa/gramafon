...
public class MainActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        configure(web);
        // lets the site tell the notification what's playing, and the notification's
        // buttons reach back into the site's own play/pause/next/prev
        web.addJavascriptInterface(new AndroidBridge(), "AndroidBridge");
        PlaybackBridge.attachActivity(this);
    ...
    /** روی نخ اصلی، جاوااسکریپت رو توی صفحه اجرا می‌کنه (اعلان دکمه‌هاش رو این‌جوری صدا می‌زنه). */
    ...
    /** صفحه‌ی وب هر بار آهنگ یا وضعیت پخش عوض بشه، اینو صدا می‌زنه (window.AndroidBridge.postState). */
    private final class AndroidBridge {
        @JavascriptInterface
        public void postState(String json) {
            // این متد روی یه نخ جدا از خود وب‌ویو صدا زده می‌شه
            runOnUiThread(() -> PlaybackBridge.updateFromJson(json));
        }
    }
    ...
    protected void onDestroy() {
        PlaybackBridge.detachActivity(this);
